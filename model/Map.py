...
def generate_map(width, height):
    ...
    Fonc
    """
    maxtrix_map = [[EMPTY for _ in range(width)] for _ in range(height)]

    # Choose random coordinates for the starting room
    start_x = random.randint(0, width - 1)
    start_y = random.randint(0, height - 1)
    maxtrix_map[start_y][start_x] = START
    ...
    maxtrix_map[final_y][final_x] = FINAL

    # Connect the starting and final rooms
    connect_rooms(maxtrix_map, start_x, start_y, final_x, final_y)
...
def connect_rooms(maxtrix_map, start_x, start_y, final_x, final_y):
    """
    ...
    while frontier:
        ...
        for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
            ...
            if 0 <= next_x < len(maxtrix_map[0]) and 0 <= next_y < len(maxtrix_map):
    ...
    while current != (start_x, start_y):
        maxtrix_map[current[1]][current[0]] = PATH
        current = came_from[current]

# Function to display the map
def display_map(maxtrix_map):
    for row in maxtrix_map:
        print(" ".join(str(cell) for cell in row))
# Example usage
width = 10
height = 10
maxtrix_map = generate_map(width, height)
display_map(maxtrix_map)
